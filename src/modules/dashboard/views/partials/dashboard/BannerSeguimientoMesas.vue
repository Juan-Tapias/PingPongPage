    ...
    >
      ...
      <div class="w-full md:w-80 max-w-full p-4 rounded-xl bg-slate-900 dark:bg-[#080d1a] text-white border border-slate-800 shadow-md shrink-0">
        ...
        <div class="mt-3 pt-2.5 border-t border-slate-800/80">
          <button
            v-if="primerPartido.id === partidoTransmitiendoId"
            type="button"
            class="w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 active:scale-98 text-white font-extrabold text-xs transition-all shadow-md shadow-emerald-950/50 cursor-pointer"
            @click="emit('abrir-transmision-general')"
          >
          ...
          <button
            v-else-if="primerPartido.transmisionActiva"
            type="button"
    ...
    <div v-else class="space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
        ...
        >
          ...
          <div class="mt-3 pt-2.5 border-t border-slate-800/80">
            <button
              v-if="partido.id === partidoTransmitiendoId"
              type="button"
              class="w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 active:scale-98 text-white font-extrabold text-xs transition-all shadow-md shadow-emerald-950/50 cursor-pointer"
              @click="emit('abrir-transmision-general')"
            >
...
interface Props {
  ...
  partidoTransmitiendoId?: string | number | null
}

const props = withDefaults(defineProps<Props>(), {
  partidos: () => [],
  partidoTransmitiendoId: null,
})
