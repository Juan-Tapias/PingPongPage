<template>
  <div class="min-h-screen w-full bg-slate-100 dark:bg-[#060a14] text-slate-800 dark:text-slate-100 flex flex-col font-sans transition-colors duration-300 relative overflow-x-hidden">
    ...
    <!-- Hero Banner de Mando Deportivo con Mesa Vertical -->
    <section class="relative w-full border-b border-slate-200/80 dark:border-slate-800/80 bg-slate-950 overflow-hidden">
      <!-- Fondo fotográfico con mesa en perspectiva vertical nítida -->
      <div
        class="absolute inset-0 bg-cover bg-center sm:bg-[center_35%] scale-100 opacity-30 dark:opacity-20 pointer-events-none"
        style="background-image: url('/images/table-vertical.jpg');"
      />

      <!-- Gradientes cinematográficos de integración -->
      <div class="absolute inset-0 bg-gradient-to-r from-[#070b16] via-[#070b16]/90 to-[#070b16]/70" />
      <div class="absolute bottom-0 inset-x-0 h-24 bg-gradient-to-t from-[#070b16] via-[#070b16]/60 to-transparent" />
      <div class="absolute -top-10 right-20 w-96 h-96 bg-orange-500/15 rounded-full blur-3xl pointer-events-none" />

      <div class="max-w-7xl 2xl:max-w-[1720px] mx-auto px-4 sm:px-6 2xl:px-8 pt-8 pb-10 relative z-10">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div class="max-w-3xl space-y-2">
            <!-- Badge Superior Oficial -->
            <div class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-orange-500/20 border border-orange-500/35 text-orange-400 text-[11px] font-black uppercase tracking-wider shadow-2xs backdrop-blur-md">
            ...
            <h1 class="text-2xl sm:text-3xl lg:text-4xl font-black font-heading tracking-tight text-white leading-tight drop-shadow-md">
              Gestión y Control de <span class="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 via-amber-400 to-orange-400">Torneos Oficiales</span>
            </h1>

            <p class="text-xs sm:text-sm text-slate-300 font-medium leading-relaxed max-w-2xl drop-shadow">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4 pt-8">
          ...
          <div class="relative p-4 sm:p-5 rounded-2xl bg-white/5 dark:bg-slate-900/60 border border-white/10 dark:border-slate-800/80 backdrop-blur-md shadow-lg shadow-black/20 flex flex-col justify-between overflow-hidden group hover:border-amber-500/40 hover:-translate-y-0.5 transition-all">
            <div class="flex items-center justify-between gap-2">
              <span class="text-[11px] font-black uppercase tracking-wider text-amber-400">
                Bolsa Acumulada
              </span>
            ...
            <div class="mt-3">
              <span class="text-xl sm:text-2xl font-black font-mono text-amber-400">
                ${{ statsCircuito.bolsaTotal.toLocaleString('es-CO') }} COP
              </span>
            </div>
            <p class="text-[11px] text-amber-300/90 mt-1 font-bold">
          ...
          <div class="relative p-4 sm:p-5 rounded-2xl bg-white/5 dark:bg-slate-900/60 border border-white/10 dark:border-slate-800/80 backdrop-blur-md shadow-lg shadow-black/20 flex flex-col justify-between overflow-hidden group hover:border-orange-500/40 hover:-translate-y-0.5 transition-all">
            <div class="flex items-center justify-between gap-2">
              ...
              <div class="w-8 h-8 rounded-xl bg-orange-500/15 text-orange-400 flex items-center justify-center border border-orange-500/30">
      ...
      <section class="space-y-6">
        ...
        <div v-if="torneosFiltrados.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-5 sm:gap-6">
          <div
            ...
            :key="torneo.id"
            class="group relative bg-white dark:bg-[#0c1222] rounded-3xl border border-slate-200/90 dark:border-slate-800/90 shadow-sm hover:shadow-xl hover:shadow-orange-500/10 hover:border-orange-500/40 dark:hover:border-orange-500/40 transition-all duration-300 flex flex-col justify-between overflow-hidden hover:-translate-y-1.5"
          >
            ...
            <!-- Marca de agua deportiva tenue -->
            <div class="pointer-events-none absolute -right-6 -bottom-6 w-36 h-36 opacity-5 dark:opacity-10 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500">
              <Trophy class="w-full h-full text-current" />
            </div>

            <!-- Contenido de la Tarjeta -->
            <div class="p-5 sm:p-6 flex-1 flex flex-col justify-between gap-4 relative z-10">
              <div class="space-y-3">
                <!-- Fila 1: Badges y Selector Rápido de Estado -->
                <div class="flex items-center justify-between gap-2">
                  <span class="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-lg bg-orange-500/15 text-orange-600 dark:text-orange-400 border border-orange-500/30">
                    <Flame class="w-3.5 h-3.5" />
                    <span>Torneo Oficial</span>
                  </span>

                  <!-- Selector estilizado de Estado -->
                  <div class="relative inline-flex items-center">
                    <select
                      :value="torneo.estado"
                      class="appearance-none text-[11px] font-bold pl-3 pr-7 py-1 rounded-xl border focus:outline-none cursor-pointer shrink-0 transition-colors shadow-2xs"
                      :class="[
                        torneo.estado === 'en curso' ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border-emerald-500/30' :
                        torneo.estado === 'por iniciar' ? 'bg-sky-500/15 text-sky-700 dark:text-sky-300 border-sky-500/30' :
                    ...
                    <ChevronDown class="w-3.5 h-3.5 absolute right-2 pointer-events-none text-current opacity-70" />
                  </div>
                </div>

                <!-- Fila 2: Nombre del Torneo -->
                <div class="mt-1">
                  <h3 class="text-base sm:text-lg font-black font-heading text-slate-900 dark:text-white tracking-tight leading-snug group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors line-clamp-2">
                    {{ torneo.nombre }}
                  </h3>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-1.5 pt-1">
                  <span class="text-[10px] font-bold px-2.5 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800/90 text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-slate-700/80">
                    {{ torneo.categoria || 'Categoría Abierta' }}
                  </span>
                  <span class="text-[10px] font-bold px-2.5 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800/90 text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-slate-700/80">
                    {{ torneo.modalidad || 'Individual Masculino' }}
                  </span>
                  <span class="text-[10px] font-bold px-2.5 py-0.5 rounded-lg bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20 flex items-center gap-1">
                ...
                <div class="p-3.5 rounded-2xl bg-slate-50/80 dark:bg-slate-900/70 border border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between gap-3 shadow-2xs">
                  <div class="flex items-center gap-2.5 min-w-0">
                    ...
                    <div class="min-w-0">
                      <span class="text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 block leading-tight truncate">
                        Bolsa 100% Campeón
                      </span>
                      <span class="text-xs sm:text-sm font-black font-mono text-amber-600 dark:text-amber-300 truncate block">
                        ${{ calcularBolsaTorneo(torneo).toLocaleString('es-CO') }} COP
                      </span>
                    </div>
                  </div>
                  <span class="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 dark:bg-emerald-950/50 px-2.5 py-1 rounded-lg border border-emerald-500/20 shrink-0">
              ...
              <div class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center gap-2">
                <button
                  type="button"
                  class="p-2.5 rounded-xl text-slate-400 hover:text-red-500 hover:bg-red-500/10 border border-transparent hover:border-red-500/20 transition-all cursor-pointer shrink-0"
                ...
                <button
                  type="button"
                  class="group/btn flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-gradient-to-r from-orange-600 via-orange-500 to-amber-500 hover:from-orange-500 hover:to-amber-400 text-white text-xs font-black uppercase tracking-wider shadow-md shadow-orange-500/25 hover:shadow-lg hover:shadow-orange-500/40 hover:-translate-y-0.5 active:translate-y-0 transition-all cursor-pointer"
                  @click="abrirDetalleTorneo(torneo)"
                >
                  <Swords class="w-3.5 h-3.5 text-white shrink-0" />
                  <span>Gestionar Torneo</span>
                  <ArrowRight class="w-3.5 h-3.5 text-white ml-auto group-hover/btn:translate-x-1 transition-transform" />
                </button>
              </div>
            </div>
          </div>
